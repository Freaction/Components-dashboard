...
app.get('/api/tokens/:version', async (req, res) => {
  ...
  try {
    ...
    // Cross-pollinate figmaIds across all modes
    const getAllTokens = (node) => {
      let tokens = [...(node.tokens || [])];
      if (node.children) {
        Object.values(node.children).forEach(child => {
          tokens = tokens.concat(getAllTokens(child));
        });
      }
      return tokens;
    };

    const tokenPathsToIds = new Map();
    Object.values(finalResult).forEach(tree => {
      const tokens = getAllTokens({ children: tree });
      tokens.forEach(t => {
        if (t.figmaId) tokenPathsToIds.set(t.path, t.figmaId);
      });
    });
    ...
    Object.values(finalResult).forEach(tree => {
      const tokens = getAllTokens({ children: tree });
      tokens.forEach(t => {
        if (!t.figmaId && tokenPathsToIds.has(t.path)) {
          t.figmaId = tokenPathsToIds.get(t.path);
        }
      });
    });

    // Remove the 'Global' mode generated by Design Tokens plugin if we have other modes from variables2json
    if (finalResult['Global'] && Object.keys(finalResult).length > 1) {
      delete finalResult['Global'];
    }
...
function startServer(port) {
  const server = app.listen(port, () => {
    ...
    // Removed auto-open browser for microservice
  }).on('error', (err) => {
